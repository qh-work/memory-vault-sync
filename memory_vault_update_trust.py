"""Publisher verification for the optional Memory Vault full client.
...
release archive. It never signs metadata and never accepts private keys.

Adapted from v0.21.0's public signed_updates verifier (Apache-2.0). The v2
profile binds the exact full-client ZIP and source commit, without a Git
runtime or release ancestry requirement.
"""
...
from memory_vault_metadata import (
    ProtocolValueError, jcs_json_bytes, persisted_json_bytes, sha256_bytes,
    strict_json_loads,
)


SIGNED_UPDATE_PROFILE = "memory-vault-tuf-style-rsa-pss/v2"
TRUST_STORE_SCHEMA = "memory-vault-update-trust-store/v2"
TRUST_SUMMARY_SCHEMA = "memory-vault-update-trust-summary/v2"
TARGET_CUSTOM_SCHEMA = "memory-vault-update-target/v2"
SPEC_VERSION = "1.0.0"
TARGET_PATH = "memory-vault-client.zip"
...
MAX_VIRTUAL_BUNDLE_BYTES = 32 * 1024 * 1024
...
def _release_version(value: Any) -> tuple[int, int, int]:
    if not isinstance(value, str) or re.fullmatch(r"[0-9]{1,4}\.[0-9]{1,4}\.[0-9]{1,4}", value) is None:
        raise SignedUpdateError("signed release version is invalid")
...
def _validate_regular_file(
    ...
    *,
    ...
    unsafe_mode = (
        ...
        and os.name != "nt"
        and (bool(stat.S_IMODE(observed.st_mode) & 0o077) or observed.st_uid != os.getuid())
    )
    if (
    ...
    *,
    ...
    try:
        if os.name == "nt":
            ...
            descriptor = open_file(path, flags, private=require_private_mode)
        else:
        ...
        if (
            ...
            or any(getattr(opened, field, 0) != getattr(opened_again, field, 0)
...
def _validate_root(envelope: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for observed_id, raw_key in signed["keys"].items():
        ...
        physical = _parse_rsa_public_key(normalized_key["keyval"]["public"])
        if physical in physical_keys:
            raise SignedUpdateError("signed root reuses one physical RSA key under multiple IDs")
...
def _validate_targets(
    ...
    if (
        ...
        or not isinstance(custom.get("source_commit_sha"), str)
        or _SOURCE_SHA_RE.fullmatch(custom["source_commit_sha"]) is None
        or set(custom["source_commit_sha"]) == {"0"}
    ...
    _release_version(custom.get("plugin_version"))
    ...
    normalized_target = {
        ...
        "custom": {
            ...
            "source_commit_sha": str(custom["source_commit_sha"]),
            "protocol": {
...
def _validate_last_target(value: Any) -> dict[str, Any] | None:
    ...
    if (
        ...
        or not isinstance(value.get("source_commit_sha"), str)
        or _SOURCE_SHA_RE.fullmatch(value["source_commit_sha"]) is None
        or set(value["source_commit_sha"]) == {"0"}
    ...
    _release_version(value.get("plugin_version"))
...
def _validate_candidate(candidate: Any) -> dict[str, Any]:
    if (
        ...
        or _SOURCE_SHA_RE.fullmatch(candidate["commit_sha"]) is None
    ):
        raise SignedUpdateError("release update candidate is invalid")
    return {
...
def verify_update_chain(
    ...
    *,
    ...
    _release_version(observed["version"])
    custom = target["custom"]
    protocol = custom["protocol"]
    if (
        custom["plugin_name"] != plugin_name
        or custom["plugin_version"] != observed["version"]
        or custom["source_commit_sha"] != observed["commit_sha"]
        ...
        raise SignedUpdateError(
            "signed target does not match the release plugin bundle"
        )
    last_target = {
        ...
        "source_commit_sha": custom["source_commit_sha"],
    ...
    previous_target = trusted["last_target"]
    if previous_target is not None:
        prior_version = _release_version(previous_target["plugin_version"])
        next_version = _release_version(last_target["plugin_version"])
        if next_version < prior_version:
            raise SignedUpdateError("signed plugin version rollback was refused")
        if next_version == prior_version and any(
            previous_target[field] != last_target[field]
            for field in ("plugin_name", "bundle_sha256", "bundle_length", "source_commit_sha")
        ):
            raise SignedUpdateError("same-version signed plugin substitution was refused")
    updated_store = {
        "schema_version": TRUST_STORE_SCHEMA,
        "profile": SIGNED_UPDATE_PROFILE,
    ...
    return {
        ...
        "observed_source_commit": observed["commit_sha"],
    }
    ...
    *,
    ...
    if value is None:
        return {
            ...
            "required": False,
            "profile": None,
            "mode": "https-and-checksum-only-not-publisher-authenticated",
            "valid": True,
            "root_version": None,
            "root_sha256": None,
            "root_expires": None,
