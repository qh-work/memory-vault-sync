...
| P04 | Core `memory.views`, `memory.graph`, `memory.reindex`; `docs/GRAPH_VIEWS.md` | Parity-repair handoff case confirms quarantined relation filtering without rewriting canonical memory. Bounded timelines/frontiers are implemented; complete claim-state/trust/pagination runtime evidence remains pending |
...
| P06 | `memory_vault_sync.py`, `memory_vault_transfer.py`, `memory_vault_dependency.py`; self-contained v2 plus stream-proven v3 | Earlier four signed-directory cases remain source-pinned. New pending-file publication/exit/exact-retry and shared-directory compatibility cases pass; these do not exercise a full signed exchange. First-use/invalidated closure budgets stay explicit; remote/group/scale/concurrent-writer acceptance remains pending |
...
| P13 | `memory_vault_storage.py` and configured client/transfer/recovery/pack/update consumers | New publication repairs include three controlled macOS child exits and in-process output/rollback boundaries without tightening explicit POSIX parent modes. Native Windows/Linux, actual unsupported filesystems and the independent core exporter remain outside this verification; existing private aliases are not auto-repaired |
