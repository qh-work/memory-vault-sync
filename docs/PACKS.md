...
the published v0.25.0 client was limited to 512 MiB. This is the v0.25.1
capacity-patch source, not a rewrite of that release or an assertion that patch
assets have already been uploaded. Existing file-pack/v1
...
The [capacity report](V0_25_PACK_CAPACITY_SMOKE.md) records one opted-in actual
516 MiB synthetic create/copy/resume/repeat/unpack/hash case passing in 3.891763
seconds on `2f67a7099e9eba0effb3483ed3a9ba3bf2f90f80`: one chunk copied first,
then four calls at the default 32-chunk budget. Separate checks accepted a
2 GiB/512-entry manifest and rejected a sparse 2 GiB + 1 byte source by stat
before output. **No full 2 GiB transfer was run.** The source remained unchanged;
no keys, network, private Vault or child processes were used. This is bounded
functional evidence, not a throughput benchmark or restoration of unbounded
copy behavior. See [patch notes](RELEASE_NOTES_V0_25_1.md).
...
memory. The capacity case above is the only newly executed pack case; other
public synthetic cases in `tests/test_v025_portable_packs.py` cover native
routing, resumable copies and unsafe-file refusals but remain **unrun** unless
separately recorded. Selected publication cases in
